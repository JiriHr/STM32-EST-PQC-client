...
func (svc *KMSServiceBackend) SignDilithium(ctx context.Context, keyID string, message []byte, mode string) ([]byte, error) {
	lFunc := chelpers.ConfigureLogger(ctx, svc.logger)

	// Get the key to find the engine ID
	key, err := svc.GetKey(ctx, services.GetKeyInput{
		Identifier: keyID,
	})
	if err != nil {
		lFunc.Errorf("failed to get key for Dilithium signing: %s", err)
	...
	if !ok {
		lFunc.Errorf("crypto engine not found: %s", key.EngineID)
		return nil, fmt.Errorf("crypto engine not found: %s", key.EngineID)
	}

	// Call the engine's SignDilithium method
	engineInstance := *engine
	signature, err := engineInstance.SignDilithium(key.KeyID, message, mode)
	if err != nil {
		lFunc.Errorf("Dilithium signing failed: %s", err)
...
func (svc *KMSServiceBackend) CreateKey(ctx context.Context, input services.CreateKeyInput) (*models.Key, error) {
	...
	//Validator change, once again relaxed check for PQC case, as the previous version had || input.Size == 0 check, which throws error. We only check for algo.
	if input.Algorithm == "" {
		lFunc.Error("algorithm")
		return nil, errs.ErrValidateBadRequest
	}

	// Handle PQC algorithms separately
	if _, ok := kmsPQCMode(input.Algorithm); ok {
		return svc.createPQCKey(ctx, input, lFunc)
	}

	// Handle classical algorithms
	if input.Size == 0 {
		lFunc.Error("size is required for classical algorithms")
		return nil, errs.ErrValidateBadRequest
	}
...
// Helper function to create PQC keys
func (svc *KMSServiceBackend) createPQCKey(ctx context.Context, input services.CreateKeyInput, lFunc *logrus.Entry) (*models.Key, error) {
	var engine *cryptoengines.CryptoEngine
	engineID := ""
	var ok bool
	...
	engineInstance := *engine
	lFunc.Debugf("Creating PQC key with algorithm %s", input.Algorithm)

	mode, ok := kmsPQCMode(input.Algorithm)
	if !ok {
		return nil, fmt.Errorf("unsupported PQC algorithm: %s", input.Algorithm)
	}

	var publicKeyBytes []byte
	keyID, publicKeyBytes, err := engineInstance.CreateDilithiumPrivateKey(ctx, mode)
	if err != nil {
		lFunc.Errorf("Error creating PQC private key: %s", err)
		return nil, errors.New("Failed to create PQC private key")
	}

	// For PQC keys, we store the public key as base64-encoded raw bytes
	base64PEM := base64.StdEncoding.EncodeToString(publicKeyBytes)
	...
	kmsKey := models.Key{
		...
		HasPrivateKey: true,
		Algorithm:     input.Algorithm,
		Size:          0, // PQC keys have fixed sizes, set elsewhere
		PublicKey:     base64PEM,
		CreationTS:    time.Now(),
		Tags:          tags,
		Metadata:      metadata,
...
func (svc *KMSServiceBackend) checkKeySpecEngineCompliance(keyType string, size int, engine cryptoengines.CryptoEngine) error {
	//PQC Keys are models.KeyType and not string, therefore we make slight adjustments to handled type
	engineConfig := engine.GetEngineConfig()
	for _, spec := range engineConfig.SupportedKeyTypes {
		if spec.Type.String() == keyType {
			//If size = 0 (PQC keys) we relax the check, can be edited later for set keysize, but its not necessary atm
			if size == 0 || slices.Contains(spec.Sizes, size) {
				return nil
			}
