...
func (r *caHttpRoutes) GetStats(ctx *gin.Context) {
	...
	caQueryParams, err := FilterQueryWithPrefix(ctx.Request, resources.CAFilterableFields, "ca_filter")
	if err != nil {
		ctx.JSON(400, gin.H{"err": err.Error()})
		return
	}
	certQueryParams, err := FilterQueryWithPrefix(ctx.Request, resources.CertificateFilterableFields, "cert_filter")
	if err != nil {
		ctx.JSON(400, gin.H{"err": err.Error()})
		return
	}
...
// @Description Create a managed certificate by generating or reusing a key and issuing it from the selected CA
// @Accept json
// @Produce json
// @Security OAuth2Password
...
// @Failure 400 {string} string "Struct Validation error || CA Status inconsistent"
// @Failure 500
// @Router /cas/{id}/certificates [post]
	...
	profile, err := r.svc.CreateIssuanceProfile(ctx.Request.Context(), services.CreateIssuanceProfileInput{
		Profile: models.IssuanceProfile{
			...
			CryptoEnforcement: models.IssuanceProfileCryptoEnforcement{
				...
				AllowDilithiumKeys:           requestBody.CryptoEnforcement.AllowDilithiumKeys,
				AllowedDilithiumModes:        requestBody.CryptoEnforcement.AllowedDilithiumModes,
				AllowMLDSAKeys:               requestBody.CryptoEnforcement.AllowMLDSAKeys,
				AllowedMLDSASecurityVersions: requestBody.CryptoEnforcement.AllowedMLDSASecurityVersions,
			},
		},
	})
