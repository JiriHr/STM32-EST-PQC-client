...
func (mw KMSEventPublisher) SignDilithium(ctx context.Context, keyID string, message []byte, mode string) ([]byte, error) {
	// Keep the middleware interface-complete for PQC signing while preserving
	// the existing behavior of delegating the operation to the wrapped service.
