...
func (mw KMSOTelTracer) SignDilithium(ctx context.Context, keyID string, message []byte, mode string) ([]byte, error) {
	// Keep PQC signing inside the same tracing envelope as the classical KMS
	// operations so future debugging has one consistent entry point.
	ctx, span := otel.GetTracerProvider().Tracer(mw.tracerName).Start(ctx, sdk.GetCallerFunctionName(), trace.WithAttributes(semconv.ServiceName(mw.serviceName)))
	defer span.End()
