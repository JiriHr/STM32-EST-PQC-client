...
func (r *kmsHttpRoutes) SignDilithium(ctx *gin.Context) {
	type uriParams struct {
		ID string `uri:"id" binding:"required"`
	}
	...
	var requestBody resources.SignDilithiumBody
	if err := ctx.BindJSON(&requestBody); err != nil {
		ctx.JSON(400, gin.H{"err": err.Error()})
		return
	}

	// Dedicated PQC signing path for remote KMS clients.
	signature, err := r.svc.SignDilithium(ctx.Request.Context(), params.ID, requestBody.Message, requestBody.Mode)
	if err != nil {
		switch err {
		case errs.ErrKeyNotFound:
			ctx.JSON(404, gin.H{"err": err.Error()})
	...
	ctx.JSON(200, models.MessageSignature{
		Signature: models.Signature(signature),
	})
}
