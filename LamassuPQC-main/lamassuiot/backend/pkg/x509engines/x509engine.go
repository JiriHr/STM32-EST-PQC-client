...
func (engine X509Engine) CreateRootCA(ctx context.Context, signer crypto.Signer, keyID string, subject models.Subject, validity models.Validity, profile models.IssuanceProfile) (*x509.Certificate, error) {
	...
	var err error
	if len(rawHex) == 0 {
		rawHex, err = helpers.SubjectKeyIDFromPublicKey(signer.Public())
		if err != nil {
			lFunc.Errorf("could not derive subject key identifier from public key: %s", err)
			return nil, err
		}
	}
	...
	algorithm, err := helpers.SigningAlgorithmFromPublicKey(signer.Public())
	if err == nil && helpers.SupportsCustomX509CertificateCreation(algorithm, signer.Public()) {
		certificate, err := helpers.CreateCertificate(&template, &template, signer.Public(), signer, algorithm)
		if err != nil {
			lFunc.Errorf("could not sign custom certificate: %s", err)
			return nil, err
		}
...
func (engine X509Engine) SignCertificateRequest(ctx context.Context, csr *x509.CertificateRequest, ca *x509.Certificate, caSigner crypto.Signer, profile models.IssuanceProfile) (*x509.Certificate, error) {
	...
	if err := helpers.VerifyCertificateRequestSignature(csr); err != nil {
		lFunc.Errorf("could not verify certificate request signature: %s", err)
		return nil, err
	}

	subjectPublicKey := csr.PublicKey
	if subjectPublicKey == nil {
		algorithm, rawPublicKey, err := helpers.ExtractPQCPublicKeyFromSPKI(csr.RawSubjectPublicKeyInfo)
		if err == nil {
			subjectPublicKey = helpers.NewPQCPublicKey(algorithm, rawPublicKey)
		}
	}

	// Validate CSR public key against crypto enforcement rules
	if err := engine.validateCryptoEnforcement(ctx, subjectPublicKey, profile.CryptoEnforcement); err != nil {
		return nil, err
	}
	...
	var err error
	if subjectPublicKey != nil {
		if helpers.SupportsCustomX509CertificateCreation("", subjectPublicKey) {
			rawHex, err = helpers.SubjectKeyIDFromSPKI(csr.RawSubjectPublicKeyInfo)
		} else {
			var skid string
			skid, err = software.NewSoftwareCryptoEngine(engine.logger).EncodePKIXPublicKeyDigest(subjectPublicKey)
			if err == nil {
	...
	caAlgorithm, err := helpers.SigningAlgorithmFromPublicKey(caSigner.Public())
	if err == nil && helpers.SupportsCustomX509CertificateCreation(caAlgorithm, subjectPublicKey) {
		certificate, err := helpers.CreateCertificate(&certificateTemplate, ca, subjectPublicKey, caSigner, caAlgorithm)
		if err != nil {
			lFunc.Errorf("could not sign custom certificate: %s", err)
			return nil, err
		}
	...
	certificateBytes, err := x509.CreateCertificate(entropy, &certificateTemplate, ca, subjectPublicKey, caSigner)
	if err != nil {
		lFunc.Errorf("could not sign certificate: %s", err)
		return nil, err
	}
...
func (engine X509Engine) GenerateCertificateRequest(ctx context.Context, csrSigner crypto.Signer, subject models.Subject) (*x509.CertificateRequest, error) {
	...
	entropy := software.NewLamassuEntropy(ctx)
	algorithm, err := helpers.SigningAlgorithmFromPublicKey(csrSigner.Public())
	if err == nil && helpers.IsPQCAlgorithm(algorithm) {
		return helpers.CreateCertificateRequest(&template, csrSigner, algorithm)
	}
...
func (engine X509Engine) validateCryptoEnforcement(
	...
	if mode, ok := helpers.DilithiumModeFromPublicKey(publicKey); ok {
		if !enforcement.AllowDilithiumKeys {
			lFunc.Errorf("public key is Dilithium, but issuance profile does not allow Dilithium keys")
			return fmt.Errorf("public key is Dilithium, but issuance profile does not allow Dilithium keys")
		}
		if enforcement.AllowedDilithiumModes != nil {
			if !slices.Contains(enforcement.AllowedDilithiumModes, mode) {
				lFunc.Errorf("public key uses Dilithium mode %d, but issuance profile does not allow this mode", mode)
				return fmt.Errorf("public key uses Dilithium mode %d, but issuance profile does not allow this mode", mode)
			}
		}
	}

	if securityVersion, ok := helpers.MLDSASecurityVersionFromPublicKey(publicKey); ok {
		if !enforcement.AllowMLDSAKeys {
			lFunc.Errorf("public key is ML-DSA, but issuance profile does not allow ML-DSA keys")
			return fmt.Errorf("public key is ML-DSA, but issuance profile does not allow ML-DSA keys")
		}
		if enforcement.AllowedMLDSASecurityVersions != nil {
			if !slices.Contains(enforcement.AllowedMLDSASecurityVersions, securityVersion) {
				lFunc.Errorf("public key uses ML-DSA-%d, but issuance profile does not allow this security version", securityVersion)
				return fmt.Errorf("public key uses ML-DSA-%d, but issuance profile does not allow this security version", securityVersion)
			}
		}
	}
