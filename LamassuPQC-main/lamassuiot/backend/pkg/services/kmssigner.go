...
func (s *kmsCryptoSigner) Public() crypto.PublicKey {
	...
	if algorithm, ok := kmsPQCX509Algorithm(s.key.Algorithm); ok {
		return helpers.NewPQCPublicKey(algorithm, b)
	}
...
func (s *kmsCryptoSigner) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	if mode, ok := kmsPQCMode(kmsKeyAlg); ok {
		// Keep PQC signing behind the service interface so middleware/decorators
		// don't break this signer later.
		return s.sdk.SignDilithium(s.ctx, s.key.KeyID, digest, mode)
	}
...
func kmsPQCMode(algorithm string) (string, bool) {
	switch algorithm {
...
func kmsPQCX509Algorithm(algorithm string) (string, bool) {
	switch algorithm {
	case "DILITHIUM2", "DILITHIUM_2":
