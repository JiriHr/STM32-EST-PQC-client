...
func (svc *CAServiceBackend) ImportCA(ctx context.Context, input services.ImportCAInput) (*models.CACertificate, error) {
	...
	isSelfSigned := false
	// Route signature validation through the helper so Dilithium-backed CAs
	// don't fall back to stdlib-only verification.
	if err := helpers.VerifyCertificateSignature(caCertX509, caCertX509); err != nil {
		isSelfSigned = false
	} else {
		isSelfSigned = true
	}
	...
	if !isSelfSigned {
		...
		findParentCAInArray := func(ca *models.X509Certificate, parentCAs []models.CACertificate) *models.CACertificate {
			...
			for _, parentCA := range parentCAs {
				...
				err = helpers.VerifyCertificateSignature(&p, &c)
	...
	for len(queue) > 0 {
		...
		svc.caStorage.SelectByIssuerAndAuthorityKeyID(ctx, parent.Certificate.Subject, parent.Certificate.SubjectKeyID, storage.StorageListRequest[models.CACertificate]{
			...
			ApplyFunc: func(child models.CACertificate) {
				...
				if err := helpers.VerifyCertificateSignature(childCertX509, childCertX509); err != nil {
					isSelfSignedChild = false
				} else {
...
func (svc *CAServiceBackend) CreateCA(ctx context.Context, input services.CreateCAInput) (*models.CACertificate, error) {
	...
	if input.ParentID == "" {
		...
		signerAlgorithm, err := helpers.SigningAlgorithmFromPublicKey(signer.Public())
		if err != nil {
			lFunc.Errorf("could not determine signer algorithm for CA %s: %s", input.Subject.CommonName, err)
			return nil, err
		}

		var csr *x509.CertificateRequest
		if helpers.IsPQCAlgorithm(signerAlgorithm) {
			csr, err = helpers.CreateCertificateRequest(csrTemplate, signer, signerAlgorithm)
		} else {
			csrDerBytes, err := x509.CreateCertificateRequest(entropy, csrTemplate, signer)
			if err == nil {
...
func (svc *CAServiceBackend) CreateCertificate(ctx context.Context, input services.CreateCertificateInput) (*models.Certificate, error) {
	...
	err := caValidator.Struct(input)
	if err != nil {
		lFunc.Errorf("CreateCertificateInput struct validation error: %s", err)
		return nil, errs.ErrValidateBadRequest
	}

	// Managed certificate creation mirrors the CA flow: create or reuse the key,
	// generate a CSR with the matching signer, then sign it through the CA path.
	var key *models.Key
	if input.KeyMetadata.KeyID == "" {
		key, err = svc.kmsService.CreateKey(ctx, services.CreateKeyInput{
			Algorithm: input.KeyMetadata.Type.String(),
			Size:      input.KeyMetadata.Bits,
			EngineID:  input.EngineID,
			Name:      fmt.Sprintf("Key For Certificate CN=%s", input.Subject.CommonName),
		})
		if err != nil {
			lFunc.Errorf("could not create key for certificate %s: %s", input.Subject.CommonName, err)
			return nil, err
		}
	} else {
		key, err = svc.kmsService.GetKey(ctx, services.GetKeyInput{
		...
		if err != nil {
			lFunc.Errorf("could not get key for certificate %s: %s", input.Subject.CommonName, err)
			return nil, err
		}
	}
	...
	csr, err := engine.GenerateCertificateRequest(ctx, signer, input.Subject)
	if err != nil {
		lFunc.Errorf("could not create CSR for certificate %s: %s", input.Subject.CommonName, err)
		return nil, err
	}

	cert, err := svc.SignCertificate(ctx, services.SignCertificateInput{
		CAID:              input.CAID,
		CertRequest:       (*models.X509CertificateRequest)(csr),
	...
	if err != nil {
		lFunc.Errorf("could not sign certificate %s: %s", input.Subject.CommonName, err)
		return nil, err
	}
	...
	cert.Type = models.CertificateTypeManaged
	cert.EngineID = key.EngineID

	if input.KeyMetadata.Type != 0 {
		cert.KeyMetadata.Type = input.KeyMetadata.Type
		cert.KeyMetadata.Bits = input.KeyMetadata.Bits
	}

	updatedCert, err := svc.certStorage.Update(ctx, cert)
	if err != nil {
		lFunc.Errorf("could not update managed certificate metadata: %s", err)
		return nil, err
	}
	...
	_, err = svc.kmsService.UpdateKeyMetadata(ctx, services.UpdateKeyMetadataInput{
		...
		Patches: chelpers.NewPatchBuilder().Add(chelpers.JSONPointerBuilder(models.KMSBindResourceKey, "-"), models.KMSBindResource{
			ResourceType: "certificate",
			ResourceID:   updatedCert.SerialNumber,
		}).Build(),
	})
	if err != nil {
		lFunc.Errorf("could not bind certificate %s to key %s: %s", updatedCert.SerialNumber, key.KeyID, err)
		return nil, err
	}
