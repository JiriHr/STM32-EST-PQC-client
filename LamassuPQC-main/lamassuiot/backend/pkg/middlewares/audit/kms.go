...
func (mw KMSAuditEventPublisher) SignDilithium(ctx context.Context, keyID string, message []byte, mode string) ([]byte, error) {
	// Keep the audit wrapper interface-complete for PQC operations; detailed
	// Dilithium audit events can be added later once the transport is settled.
	return mw.next.SignDilithium(ctx, keyID, message, mode)
}
