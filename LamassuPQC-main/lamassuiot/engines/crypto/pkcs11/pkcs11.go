...
// PQC methods - not supported by PKCS11
func (engine *pkcs11EngineContext) CreateDilithiumPrivateKey(ctx context.Context, mode string) (string, []byte, error) {
	return "", nil, fmt.Errorf("Dilithium key generation is not supported by PKCS11 engine")
}

func (engine *pkcs11EngineContext) GetDilithiumPrivateKeyByID(keyID string) ([]byte, error) {
	return nil, fmt.Errorf("Dilithium keys are not supported by PKCS11 engine")
}

func (engine *pkcs11EngineContext) SignDilithium(keyID string, data []byte, mode string) ([]byte, error) {
	return nil, fmt.Errorf("Dilithium signing is not supported by PKCS11 engine")
}
