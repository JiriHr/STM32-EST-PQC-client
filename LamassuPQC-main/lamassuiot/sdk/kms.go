...
func (cli *httpKMSClient) SignDilithium(ctx context.Context, keyID string, message []byte, mode string) ([]byte, error) {
	// Remote PQC signing mirrors the in-process KMS behavior through a small
	// dedicated endpoint so CA creation can use the HTTP SDK transparently.
	response, err := Post[*models.MessageSignature](ctx, cli.httpClient, cli.baseUrl+"/v1/keys/"+keyID+"/sign-dilithium", resources.SignDilithiumBody{
		Message: message,
		Mode:    mode,
	}, map[int][]error{})
	if err != nil {
