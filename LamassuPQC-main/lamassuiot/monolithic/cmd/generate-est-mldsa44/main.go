...
func main() {
	...
	outPath := "/tmp/lamassu-est-mldsa44-cert.pem"
	tmpDir, err := os.MkdirTemp("", "lamassu-est-mldsa44-*")
	if err != nil {
		panic(err)
	}
	...
	serverCert, err := chelpers.GenerateSelfSignedCertificate(serverKey, "lamassu-est-mldsa44.local")
	if err != nil {
		panic(err)
	}
	...
	profile, err := (*caSvc).CreateIssuanceProfile(ctx, services.CreateIssuanceProfileInput{
		Profile: models.IssuanceProfile{
			Name:        "mldsa44-est-device-profile",
			Description: "POC profile for EST-issued ML-DSA-44 device certificates",
			...
			CryptoEnforcement: models.IssuanceProfileCryptoEnforcement{
				Enabled:                      true,
				AllowMLDSAKeys:               true,
				AllowedMLDSASecurityVersions: []int{44},
			},
		},
	})
	if err != nil {
	...
	ca, err := (*caSvc).CreateCA(ctx, services.CreateCAInput{
		ID:           "mldsa44-est-ca",
		KeyMetadata:  models.KeyMetadata{Type: models.KeyTypeMLDSA44, Bits: 0},
		Subject:      models.Subject{CommonName: "ML-DSA-44 EST CA"},
		CAExpiration: models.Validity{Type: models.Duration, Duration: models.TimeDuration(24 * time.Hour)},
		ProfileID:    profile.ID,
		Metadata:     map[string]any{"poc": true, "algorithm": "ML-DSA-44"},
	})
	if err != nil {
	...
	deviceID := "mldsa44-est-device-001"
	_, err = (*dmsSvc).CreateDMS(ctx, services.CreateDMSInput{
		ID:       dmsID,
		Name:     "ML-DSA-44 EST POC",
		Metadata: map[string]any{"poc": true, "algorithm": "ML-DSA-44"},
	...
	material, err := helpers.CreateMLDSACertificateRequest(models.Subject{CommonName: deviceID}, pqc.MLDSA44)
	if err != nil {
		panic(fmt.Errorf("create ML-DSA-44 CSR: %w", err))
	}
	...
	if algorithm != "ML_DSA_44" {
		panic(fmt.Errorf("expected ML_DSA_44 public key, got %s", algorithm))
	}
