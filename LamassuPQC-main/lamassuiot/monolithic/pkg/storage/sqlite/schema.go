...
func initializeSchema(db *gorm.DB) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS issuance_profiles (
			...
			crypto_enforcement_allow_dilithium_keys INTEGER NOT NULL DEFAULT 1,
			crypto_enforcement_allowed_dilithium_modes TEXT DEFAULT '{}',
			crypto_enforcement_allow_mldsa_keys INTEGER NOT NULL DEFAULT 1,
			crypto_enforcement_allowed_mldsa_security_versions TEXT DEFAULT '{}',
			PRIMARY KEY (id)
		)`,
